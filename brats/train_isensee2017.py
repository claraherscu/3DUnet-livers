...
config = dict()
config["image_shape"] = (512, 512, 50)  # This determines what shape the images will be cropped/resampled to.
config["patch_shape"] = None  # switch to None to train on the whole image
config["labels"] = (1, 2, 4)  # the label numbers on the input image
config["n_base_filters"] = 16
config["n_labels"] = len(config["labels"])
config["all_modalities"] = ["CT"]  #  ["t1", "t1Gd", "flair", "t2"]
...
config["data_file"] = os.path.abspath("liver_data_isensee.h5")
...
def fetch_training_data_files(return_subject_ids=False):
    ...
    subject_ids = list()
    for subject_dir in glob.glob(os.path.join(os.path.dirname(__file__), "data", "preprocessed", "volume-11*")):
